...
// TODO: Use the 'slug' parameter to fetch project-specific data from your database or CMS.

type Props = { params: Promise<{ slug: string }> };

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const title = slug.replace(/-/g, ' ') + ' | Case Studies | The Earth Atelier';
  return { title };
}

export default async function CaseStudyTemplatePage({ params }: Props) {
  const { slug } = await params;

  return (
    <>
      {/* TASK 1: Hero & Overlapping Context Area - stacks on mobile to prevent overlap */}
      <div className="pt-24 pb-12 md:pt-40 md:pb-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* The Overlapping Grid - relative flow on mobile, absolute on desktop */}
        <div className="relative max-w-5xl mx-auto mt-8 mb-16 md:mb-32">
          {/* Main Image Placeholder */}
          <div className="w-full md:w-[80%] ml-auto aspect-video bg-brand-aluminium/20 rounded-lg" />
